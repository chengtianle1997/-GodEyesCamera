...
void EncoderMJPEGInit(EncoderParam encoderparam)
{
	...
	//sprintf(Filename, "%s\\Timestamp.csv", FilePath);

	sprintf(Filename, "%s\\TimeStamp_%s.csv", encoderparam.filepath, encoderparam.SerialNum);

	mjpegtimestamp.open(Filename, ios::out);
...
void SendDataToCsServer() {
	//if (GetImage0) {
		...
		PackageData0.Framecnt =  Buffer0Time.wHour * 60 * 60 * 1000 + Buffer0Time.wMinute * 60 * 1000 + Buffer0Time.wSecond * 1000 + Buffer0Time.wMilliseconds;
	...
	while (!CalExit) {
		...
		if (GetImage0 && !CalEnd0 && !Buffer0Mutex)
		{
			...
			if (EnableSendData)
			{
				...
				//Change the Framecount to Timestamp(milliseconds_count from 00:00:00:000)
				//PackageData0.Framecnt = Buffer0Info.nFrameNum;
				PackageData0.Framecnt =  Buffer0Time.wHour * 60 * 60 * 1000 + Buffer0Time.wMinute * 60 * 1000 + Buffer0Time.wSecond * 1000 + Buffer0Time.wMilliseconds;
		...
		else if (GetImage1 && !CalEnd1 && !Buffer1Mutex) 
		{
			...
			if (EnableSendData) 
			{
				...
				PackageData1.Framecnt = Buffer1Time.wHour * 60 * 60 * 1000 + Buffer1Time.wMinute * 60 * 1000 + Buffer1Time.wSecond * 1000 + Buffer1Time.wMilliseconds;
				for (int i = 0; i < ImageHeight; i++)
				{
					PackageData1.s[i] = Calparam.point[i].s;
					//PackageData1.ay[i] = Calparam.point[i].ay;
...
void AcqImageThread()
{
	...
	int sepcounter = 1;
	//置信区间范围
	float believerange = 1.3;
	//亮度数组
	float avebrightness[6];
	//区间数据条数
	int rangecounter = 0;
		...
		if (ret == MV_OK) {
			...
			else if (CameraType == 0 && method == 2)
			{
				...
				if (CalEnd0)
				{
					...
					//Rect the mat and cal the average brightness
					Rect rect(0, 0, camerainitparam.in_w, camerainitparam.in_h);

					Mat RoiImage = matsep(rect);

					cv::Scalar AverPic = cv::mean(RoiImage);

					avebrightness[rangecounter] = AverPic[0];
					...
					if (rangecounter > 5)
					{
						...
						{
							//计算总和
							if (avebrightness[in] > 0)
							{
								sumavebr += avebrightness[in];

								validcounter++;
							}

							if (avebrightness[in] > maxbri)
							{
								maxbri = avebrightness[in];

								maxpos = in;
							}
						}
						//寻找其他位置亮度平均值
						float avebr = (sumavebr - maxbri) / (validcounter - 1);
						//确认最大位置可信
						if (maxbri > believerange)
						{
							//更新阈值
							//sep = (sepcounter * sep + maxbri) / (sepcounter + 1);
							sep = (sep + maxbri) / 2;
							//更新计数器
							sepcounter++;
						}
...
int main(int argc,char* argv[])
{
	...
	if (args.exist("function"))
	{
		...
		case 10:
		{
			//Designed for CalCam  Acq + Cal + Enc
			CalEnable = 1;
			EncodeEnable = 1;
			EnableSendData = 1;
			//Image Transition
